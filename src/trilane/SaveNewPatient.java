...
/**
 * Servlet implementation class SaveNewPatient
 */
@WebServlet("/SaveNewPatient")
public class SaveNewPatient extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SaveNewPatient(request,response);
	}
	...
	protected void SaveNewPatient(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		...
		try
		{
			...
			//System.out.println("patient name no space:" + patientNameNoSpace);
			...
			if (!(rsExists.next()))
			{
				//save patient name in patient table
				
				 stSavePatient = connection.createStatement();
				
				 String sqlQuery = "insert into patient (patient_name,nhi) values ('" + patientName + "','" + patientNHI  + "')";
				 int i = stSavePatient.executeUpdate(sqlQuery);
				
			}
			/*
			//get the patient id
			stGetPatientId = connection.createStatement();
			//String sqlQueryGetId = "select patient_id from patient where  lower(replace(patient_name,' ','')) = lower('" +  patientNameNoSpace + "')" ;
			
			String sqlQueryGetId = "select patient_id from patient where  lower(replace(nhi,' ','')) = lower('" +  patientNHI + "')";
			
			rsGetPatientId = stGetPatientId.executeQuery(sqlQueryGetId);
			
			int patientId = -1;
			if(rsGetPatientId.next())
			{ 
				patientId = rsGetPatientId.getInt("patient_id");
			...
			*/
			...
			Date dateofEntryPatient = formatter.parse(dateofEntry);
			
			Date sqlDate = new Date(dateofEntryPatient.getTime());
			java.sql.Timestamp now = new java.sql.Timestamp(dateofEntryPatient.getTime());
			*/
			
			
			//code to save the patient record in procedure_on_patient table
			 stSavePatientRecord = connection.createStatement();
			 
			String sqlQuerySave = "insert into procedure_on_patient (proc_date,patient_name,patient_nhi,con_name,medproc_names,primary_operator_angio,indication_names,access_name,arterial,venous,final_impression,vessels_diseased,intervention,primary_op,bms,des,deb,lms,lad,cx,rca,ramus,graft,branch_vessel,additional_proc,lvgram,aortogram,bypass_angio,rhc,hocm_assessment,ivus,oct,ffr,angioseal,proglide,balloon_pump,angio_sculpt,rotablation,pami,complex_cto,complex_bifuc,complications,complication_desc,comments) values ('" + dateofEntry  + "','" + patientName  + "','" + patientNHI + "','" + consultant_name + "','" + procedureNames + "'," + primaryAngioOperator + ",'" + indicationNames + "','" + access + "'," + arterial + "," + venous + ",'" + finalImpressionNames + "','" + diseasedVessels + "','" + interventionNames + "'," + primaryOperator + "," + numBMS + "," + numDES + ","  + numDEB + "," + numLMS + "," + numLAD + "," + numCx + "," + numRCA + "," + ramus + "," + graft + "," + branchVessel + "," + additionalProcedure + "," + lvGram + "," + aortogram + "," + byPassAngio + "," + rhc + "," + hocm + "," + ivus + "," + oct + "," + ffr + "," + angioseal + "," + proglide + "," + balloonPump + "," + angioSculpt + "," + rotablation + "," + pami + "," + complexCto + "," + complexBifucation + "," + complications + ",'" + complicationDescription + "','" + comment + "')";																																																																																																																																																																																 
			//System.out.println(sqlQuerySave);
			 //System.out.println("the query is: " + sqlQuerySave );
			 int i = stSavePatientRecord.executeUpdate(sqlQuerySave);
			 
			if (i > 0)
			{
				
				response.getWriter().write("New Patient saved");
